...
namespace UnityEngine.Splines
{
    ...
    public class SplineAnimate : SplineComponent
    {
        ...
        #region Checkpoint Functionality
        [Header("Checkpoint Settings")]
        [SerializeField, Tooltip("World-space distance from a spline knot at which the object begins to decelerate to a stop.")]
        private float m_CheckpointDecelerationDistance = 1f;
        [SerializeField, Tooltip("Rate at which the object decelerates (and later accelerates) when approaching a checkpoint.")]
        private float m_CheckpointDecelerationRate = 1f;

        // Internal state for checkpoint control
        private bool m_PausedAtCheckpoint = false;
        // This multiplier (normally 1) will be reduced toward 0 when decelerating.
        private float m_CurrentTimeSpeed = 1f;
        // Checkpoint parameters (normalized t values along the spline) corresponding to each knot.
        private float[] m_CheckpointParameters;
        // Index of the next checkpoint (knot) to stop at.
        ...
        void OnEnable()
        {
            ...
            SetupCheckpoints(); // Initialize checkpoint data based on spline knots.
        ...
        public void Restart(bool autoplay)
        {
            ...
            // Reset checkpoint state when restarting.
            m_PausedAtCheckpoint = false;
...
#endif
            // --- Checkpoint Pause/Resume Logic ---
            if (m_PausedAtCheckpoint)
            {
                if (Input.GetKeyDown(KeyCode.Space))
                {
                    m_PausedAtCheckpoint = false;
                    // Begin accelerating from a standstill
                    m_CurrentTimeSpeed = 0f;
                }
                else
                {
                    return; // Remain paused until Space is pressed.
                }
            }

            // Determine current spline parameter (taking into account any start offset).
            float currentT = GetLoopInterpolation(true);

            // If checkpoint data exists and we have a next checkpoint...
            if (m_CheckpointParameters != null && m_CurrentCheckpointIndex < m_CheckpointParameters.Length)
            {
                float checkpointT = m_CheckpointParameters[m_CurrentCheckpointIndex];
                Vector3 checkpointPos = m_Target.EvaluatePosition(m_SplinePath, checkpointT);
                float distanceToCheckpoint = Vector3.Distance(transform.position, checkpointPos);

                if (distanceToCheckpoint < m_CheckpointDecelerationDistance)
                {
                    // Decelerate time progression.
                    m_CurrentTimeSpeed = Mathf.MoveTowards(m_CurrentTimeSpeed, 0f, m_CheckpointDecelerationRate * dt);
                    if (Mathf.Approximately(m_CurrentTimeSpeed, 0f))
                    {
                        // Snap exactly to the checkpoint.
                        m_ElapsedTime = checkpointT * m_Duration;
                        m_NormalizedTime = checkpointT;
                        m_PausedAtCheckpoint = true;
                        m_CurrentCheckpointIndex++;
                        return; // Skip further updating until resumed.
                    }
                }
                else
                {
                    // Accelerate back to normal time progression.
                    m_CurrentTimeSpeed = Mathf.MoveTowards(m_CurrentTimeSpeed, 1f, m_CheckpointDecelerationRate * dt);
                }
            }
            else
            {
                // No more checkpoints—use normal speed.
                m_CurrentTimeSpeed = 1f;
            }

            float effectiveDt = dt * m_CurrentTimeSpeed;
            CalculateNormalizedTime(effectiveDt);
            UpdateTransform();
        }
        ...
        /// <summary>
        /// Sets up the checkpoint data by computing normalized parameters for each knot.
        /// Assumes the first spline in the container is used.
        /// </summary>
        private void SetupCheckpoints()
        {
            if (m_Target != null && m_Target.Splines != null && m_Target.Splines.Count > 0)
            {
                // Attempt to get the number of knots from the first spline.
                // (This depends on your Spline API—adjust if necessary.)
                int knotCount = m_Target.Splines[0].Count;
                if (knotCount < 2)
                {
                    m_CheckpointParameters = new float[] { 0f };
                }
                else
                {
                    m_CheckpointParameters = new float[knotCount];
                    for (int i = 0; i < knotCount; i++)
                    {
                        m_CheckpointParameters[i] = (float)i / (knotCount - 1);
                    }
                }
                m_CurrentCheckpointIndex = 1;
            }
        ...
        {
            RecalculateAnimationParameters();
            SetupCheckpoints(); // Update checkpoint data if the spline changes.
        }
