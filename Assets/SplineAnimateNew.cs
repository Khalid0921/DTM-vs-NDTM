...
namespace UnityEngine.Splines
{
    ...
    /// Modified to pause at specified stop–sign positions along the spline.
    /// </summary>
    ...
    public class SplineAnimate : SplineComponent
    {
        ...
        /// <summary>
        /// Describes the different methods that can be used to animate an object along a spline.
        /// </summary>
        public enum Method
        {
        ...
        ////////////// NEW: Stop Sign Fields //////////////////
        [Header("Stop Sign Settings")]
        [Tooltip("Normalized positions (0–1) along the spline where the car should stop.")]
        public float[] stopSignPositions;

        [Tooltip("Threshold (in normalized spline units) within which the car will be considered at a stop sign.")]
        public float stopSignThreshold = 0.02f;

        // Flag to indicate whether we are stopped at a stop sign.
        private bool m_StoppedAtStopSign = false;
        // Optional event for additional logic when a stop sign is reached.
        public event Action StopSignReached;
        /////////////////////////////////////////////////////////
        ...
        #region Public Properties & Obsolete API
        ...
        /// <summary> The maxSpeed (in Unity units/second) that the Spline traversal will advance in. </summary>
        ...
        public void Play()
        {
            ...
            // Clear any stop sign pause flag when starting.
            m_StoppedAtStopSign = false;
            m_Playing = true;
#if UNITY_EDITOR
        ...
        public void Restart(bool autoplay)
        {
            ...
            // Clear any stop sign pause flag when restarting.
            m_StoppedAtStopSign = false;
            m_Playing = false;
            m_ElapsedTime = 0f;
            NormalizedTime = 0f;
        ...
        /// Modified so that if a stop sign has been reached, the car stays in place.
        /// </summary>
        public void Update()
        {
            if (!m_Playing)
...
#endif
            // Only update the progress if we are not currently stopped at a stop sign.
            if (!m_StoppedAtStopSign)
            {
                CalculateNormalizedTime(dt);
                UpdateTransform();
            }

            // Check if we are near any stop sign.
            // We use the “local” normalized time (within the current loop) for this purpose.
            float localT = GetLoopInterpolation(false);
            if (!m_StoppedAtStopSign && stopSignPositions != null)
            {
                foreach (float stopPos in stopSignPositions)
                {
                    if (Mathf.Abs(localT - stopPos) <= stopSignThreshold)
                    {
                        m_StoppedAtStopSign = true;
                        m_Playing = false;
                        StopSignReached?.Invoke();
                        Debug.Log("Stop sign reached at t = " + localT, this);
                        break;
                    }
                }
            }
        ...
        /// <summary>
        /// Call this method (for example, from a UI button) to resume animation after stopping at a stop sign.
        /// </summary>
        public void ResumeFromStopSign()
        {
            if (m_StoppedAtStopSign)
            {
                m_StoppedAtStopSign = false;
                m_Playing = true;
                Debug.Log("Resuming from stop sign", this);
            }
        }
        ...
        void UpdateTransform()
        {
            ...
            EvaluatePositionAndRotation(out Vector3 position, out Quaternion rotation);
        ...
        {
            float t = GetLoopInterpolation(true);
            position = m_Target.EvaluatePosition(m_SplinePath, t);
            rotation = Quaternion.identity;

            // Correct forward and up vectors based on axis remapping parameters
            Vector3 remappedForward = GetAxis(m_ObjectForwardAxis);
            Vector3 remappedUp = GetAxis(m_ObjectUpAxis);
            ...
            if (m_AlignmentMode != AlignmentMode.None)
            {
                ...
                switch (m_AlignmentMode)
                {
                    ...
                    case AlignmentMode.SplineObject:
                        Quaternion objectRotation = m_Target.transform.rotation;
        ...
        {
            float t = 0f;
            float normalizedTimeWithOffset = NormalizedTime + (offset ? m_StartOffsetT : 0f);
